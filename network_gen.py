...
def main(conf_file, n_networks):
    # Get network configuration info
    with open(conf_file, "r") as f:
        conf = json.load(f)
    conf_path = f"graphs/{conf['network_gen_fn']}-{conf['e_samples']}es-{conf['n_per_group']}n-{conf['p_rel']}p"
...
if __name__ == "__main__":
    if (args_count := len(sys.argv)) > 3:
        ...
        print(
            "You must specify the configuration file and the number of networks you want to generate."
        )
        raise SystemExit(2)
