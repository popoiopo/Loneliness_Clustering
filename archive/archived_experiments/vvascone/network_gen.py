...
if __name__ == "__main__":
    ...
    if (args_count := len(sys.argv)) > 4:
        logging.debug(f"Two arguments expected, got {args_count - 1}.")
        raise SystemExit(2)
    elif args_count < 3:
        logging.debug(
    ...
    if not os.path.exists(conf_file):
        logging.debug(f"Given configuration file does not exits: {conf_file}")
        raise SystemExit(2)

    if not os.path.exists(output_folder):
        logging.debug(f"Given output folder does not exits: {output_folder}")
        raise SystemExit(2)
